...
async function adminRoute(req: Request, url: URL, parts: string[], client: SupabaseClient) {
  ...
  if (parts[2] === "tokens" && parts[3] === "reset" && req.method === "POST") {
    const cfg = await config(client); const version = randomToken(); const token = randomToken();
...
async function createEvent(req: Request, client: SupabaseClient) {
  ...
  const id = id26(); const slug = `${slugify(name)}-${id.slice(-6).toLowerCase()}`; const token = randomToken(); const cfg = await config(client);
  const row = { id, slug, name, description: String(body.description || "").trim(), access_token_hash: await tokenHash(token, cfg.token_pepper), access_token_version: randomToken(), organizer_token_hash: "", guest_url: "", mode: body.mode || "delayed_reveal", status: "open", starts_at: starts, ends_at: ends, reveal_at: reveal, max_guests: body.max_guests || 250, max_photos_per_guest: body.max_photos_per_guest || 12, allow_gallery_uploads: body.allow_gallery_uploads ?? true, prefer_camera_capture: body.prefer_camera_capture ?? true, allow_immediate_gallery: body.allow_immediate_gallery ?? false, auto_approve_photos: body.auto_approve_photos ?? true, offline_upload_grace_hours: body.offline_upload_grace_hours ?? 24 };
...
async function eventDetail(client: SupabaseClient, id: string) { const [{data:event,error},{data:guests},{data:photos},stats] = await Promise.all([client.from("events").select(eventColumns).eq("id",id).single(),client.from("guests").select("id,event_id,display_name,upload_count,message_count,created_at,last_seen_at,status").eq("event_id",id).order("created_at"),client.from("photos").select("id,event_id,guest_id,object_key,content_type,size_bytes,message,status,is_developed,created_at,updated_at").eq("event_id",id).neq("status","deleted").order("created_at",{ascending:false}),eventStats(client,id)]); if(error)throw error; return {event,guest_url:guestURL(event.slug,""),guests:guests||[],photos:await signedPhotos(client,photos||[]),stats:{events:1,open_events:event.status==="open"?1:0,upcoming_events:0,guests:stats.guest_count,photos:stats.photo_count,pending_photos:stats.pending_photos,storage_bytes:stats.storage_bytes}}; }
...
async function galleryRoute(req: Request, slug: string, client: SupabaseClient) { const token=bearer(req); const event=await validEvent(client,slug,token); if(!galleryAvailable(event))throw new HTTPError(403,"Gallery is not available yet","gallery_locked"); const {data,error}=await client.from("photos").select("id,event_id,guest_id,object_key,content_type,size_bytes,message,status,is_developed,created_at,updated_at").eq("event_id",event.id).eq("status","approved").order("created_at",{ascending:false}); if(error)throw error; return json({event,photos:await signedPhotos(client,data||[])}); }
...
async function signedPhotos(client:SupabaseClient,photos:any[]){return Promise.all(photos.map(async p=>{const {data}=await client.storage.from(bucket).createSignedUrl(p.object_key,900);return{...p,public_url:data?.signedUrl};}));}
...
function guestURL(slug:string,token:string){const base=Deno.env.get("PUBLIC_WEB_URL")||"";return `${base}/guest/${slug}${token?`?t=${encodeURIComponent(token)}`:""}`;}
