...
async function eventDetail(client: SupabaseClient, id: string) { const [{data:event,error},{data:guests},{data:photos},stats] = await Promise.all([client.from("events").select(`${eventColumns},access_token_version`).eq("id",id).single(),client.from("guests").select("id,event_id,display_name,upload_count,message_count,created_at,last_seen_at,status").eq("event_id",id).order("created_at"),client.from("photos").select("id,event_id,guest_id,object_key,content_type,size_bytes,message,status,is_developed,created_at,updated_at").eq("event_id",id).neq("status","deleted").order("created_at",{ascending:false}),eventStats(client,id)]); if(error)throw error; const token=await ensureEventToken(client,event); delete event.access_token_version; return {event,guest_url:guestURL(event.slug,token),guests:guests||[],photos:await signedPhotos(client,photos||[]),stats:{events:1,open_events:event.status==="open"?1:0,upcoming_events:0,guests:stats.guest_count,photos:stats.photo_count,pending_photos:stats.pending_photos,storage_bytes:stats.storage_bytes}}; }
