...
async function signedPhotos(client:SupabaseClient,photos:any[]){return Promise.all(photos.map(async p=>{const original=await client.storage.from(bucket).createSignedUrl(p.object_key,3600);if(original.error)throw original.error;let thumbnail=original;if(canTransformImage(p.content_type)){const transformed=await client.storage.from(bucket).createSignedUrl(p.object_key,86400,{transform:{width:384,quality:70,resize:"cover"}});if(!transformed.error)thumbnail=transformed;}return{...p,public_url:original.data?.signedUrl,thumbnail_url:thumbnail.data?.signedUrl};}));}
