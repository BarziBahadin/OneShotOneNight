...
async function guestRoute(req: Request, parts: string[], client: SupabaseClient) {
  ...
  if (action === "photos" && req.method === "POST") {
    const cfg=await config(client); const {data:intent}=await client.from("upload_intents").select("photo_id,guest_id,object_key,content_type,size_bytes,token_hash,expires_at,used").eq("photo_id",body.photo_id).eq("guest_id",guest.id).eq("used",false).gt("expires_at",new Date().toISOString()).maybeSingle();
    const suppliedTokenHash=await tokenHash(String(body.upload_token||""),cfg.token_pepper);
    if(!intent||!safeEqual(intent.token_hash,suppliedTokenHash))throw new HTTPError(403,"Invalid upload token","forbidden");
